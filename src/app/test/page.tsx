...
export default function TestPage() {
    ...
    const handleYoutube = async (langOverride?: string) => {
        ...
        setYtLoading(true);
        if (!langOverride) setYtResult(""); // Reset if not just changing lang (but keep if switching)

        const langCode = langOverride || ytLanguage;

        // Extract Video ID
        const match = ytUrl.match(/(?:v=|youtu\.be\/|\/embed\/|\/v\/)([a-zA-Z0-9_-]{11})/);
        const videoId = match ? match[1] : null;

        if (!videoId) {
            setYtResult("Invalid YouTube URL");
            setYtLoading(false);
            return;
        }
        ...
        for (const instance of PIPED_INSTANCES) {
            try {
                console.log(`[Piped] Fetching from ${instance} for ${videoId}`);
                ...
                if (subtitles.length > 0) {
                    ...
                    setAvailableLanguages(subtitles.map((s: any) => ({
                        code: s.code,
                        name: s.name,
                    ...
                    // Pick best subtitle
                    let sub = subtitles.find((s: any) => s.code === langCode && !s.autoGenerated);
                    if (!sub) sub = subtitles.find((s: any) => s.code === langCode);
                    if (!sub) sub = subtitles.find((s: any) => s.code === 'en');
                    if (!sub) sub = subtitles[0]; // Fallback

                    if (sub) {
                        console.log(`[Piped] Found subtitle: ${sub.name} (${sub.code})`);
                        const vttRes = await fetch(sub.url);
                        if (!vttRes.ok) throw new Error("VTT download failed");
                        const vttText = await vttRes.text();

                        // Parse VTT (Client Side)
                        ...
                        // Split by double newline to separate cues
                        const cues = vttText.split(/\n\n/);
                        for (const cue of cues) {
                            const tm = timeRegex.exec(cue);
                            if (tm) {
                                const start = vttTimeToSeconds(tm[1]);
                                const end = vttTimeToSeconds(tm[2]);
                                let raw = cue.replace(timeRegex, '').replace(/WEBVTT.*/, '').replace(/^\d+$/, '').trim();
                                const text = raw.replace(/<[^>]*>/g, '').replace(/\n/g, ' ').trim();
                                if (text) items.push({ text, start, duration: end - start });
                            }
                        }
                        ...
                        if (items.length > 0) {
                            setYtResult(items);
                            success = true;
                            // Update current language name for UI
                            setCurrentLangName(sub.name);
                            break;
                        }
                    }
                }
            } catch (e: any) {
                console.warn(`[Piped] Failed ${instance}:`, e);
